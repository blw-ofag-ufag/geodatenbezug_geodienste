...
public interface IGeodiensteApi
{
    ...
    /// </summary>
    Task<HttpResponseMessage> StartExport(BaseTopic topic, Canton canton, string token, int attempts = 0);

    /// <summary>
    /// Checks the status of the export of a topic at geodienste.ch and returns the download URL if the export is complete.
    /// </summary>
    Task<HttpResponseMessage> CheckExportStatus(BaseTopic topic, Canton canton, string token, int attempts = 0);
}
